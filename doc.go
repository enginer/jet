...
Use the command bellow to add jet as a dependency into go.mod project:
	...
	$ go get -u github.com/enginer/jet/v2

Jet generator can be installed in one of the following ways:

 1. (Go1.16+) Install jet generator using go install:
    go install github.com/enginer/jet/v2/cmd/jet@latest

 2. Install jet generator to GOPATH/bin folder:
    cd $GOPATH/src/ && GO111MODULE=off go get -u github.com/enginer/jet/cmd/jet

 3. Install jet generator into specific folder:
    git clone https://github.com/enginer/jet.git
...
To write postgres SQL queries we import:
	...
	. "github.com/enginer/jet/v2/postgres" // Dot import is used so that Go code resemble as much as native SQL. It is not mandatory.
...
found at project wiki page - https://github.com/enginer/jet/wiki.
*/
package jet
